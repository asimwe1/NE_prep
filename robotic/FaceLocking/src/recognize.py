...
from .runtime_utils import open_camera, require_file
...
def main():
    parser = argparse.ArgumentParser(description="Run local face recognition and face locking.")
    parser.add_argument("--camera-index", type=int, default=None, help="Preferred camera index.")
    parser.add_argument("--threshold", type=float, default=0.40, help="Cosine distance threshold.")
    args = parser.parse_args()

    db_path = Path("data/db/face_db.npz")
    os.makedirs("logs", exist_ok=True)
    require_file("models/embedder_arcface.onnx", "ArcFace embedder model")
    require_file("models/face_landmarker.task", "MediaPipe face landmarker model")
    ...
    matcher = FaceDBMatcher(db=db, dist_thresh=args.threshold)
