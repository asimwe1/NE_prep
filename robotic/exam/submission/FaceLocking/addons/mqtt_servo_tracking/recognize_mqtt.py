...
from src.runtime_utils import open_camera
...
def _face_center_from_kps(kps: np.ndarray) -> Tuple[float, float]:
    return float(np.mean(kps[:, 0])), float(np.mean(kps[:, 1]))


def _enhance_low_light_bgr(frame_bgr: np.ndarray) -> np.ndarray:
    """
    Improve local contrast for dark indoor frames before face detection.
    """
...
class ArcFaceEmbedderONNX:
    ...
    Input: 112x112 BGR -> internally RGB + (x-127.5)/128.
    Output: (1,D) or (D,)
    """
    def __init__(
        self,
        ...
        if self.debug:
            ...
            print("[embed] input:", self.input_meta.name, self.input_meta.shape, self.input_meta.type)
            print("[embed] layout:", self.input_layout)
    ...
    def _infer_embedding(self, aligned_bgr_112: np.ndarray) -> np.ndarray:
    ...
    def embed(self, aligned_bgr_112: np.ndarray) -> np.ndarray:
        emb = self._infer_embedding(aligned_bgr_112)
        emb_flip = self._infer_embedding(cv2.flip(aligned_bgr_112, 1))
...
class HaarFaceMesh5pt:
    ...
    def detect(self, frame_bgr: np.ndarray, max_faces: int = 5) -> List[FaceDet]:
        ...
        enhanced_bgr = _enhance_low_light_bgr(frame_bgr)
        ...
        for (x, y, w, h) in faces:
            ...
            roi = enhanced_bgr[ry1:ry2, rx1:rx2]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.6,
        help="How long to keep the last tracking command during brief recognition dropouts before switching to SEARCH.",
    )
    parser.add_argument(
        "--reacquire-confirm-frames",
        type=int,
        default=2,
        help="How many consecutive locked-face frames are required to exit SEARCH after reacquiring the target.",
    )
    parser.add_argument(
        "--max-lock-jump-px",
        type=float,
        default=140.0,
        help="Maximum allowed center jump for a locked face between frames before that frame is ignored as unstable.",
    )
    parser.add_argument(
        "--search-when-no-faces-only",
        action="store_true",
        help="Only enter SEARCH when no faces are visible at all. If a face is still visible but recognition flickers, keep tracking/centering instead of sweeping.",
    )
    parser.add_argument(
        "--mqtt-min-interval",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Preferred camera index. Uses the same auto-fallback logic as enrollment.",
    )
...
def main():
    ...
    args.reacquire_confirm_frames = int(max(1, args.reacquire_confirm_frames))
    args.max_lock_jump_px = float(max(20.0, args.max_lock_jump_px))
    ...
    try:
        cap, camera_index = open_camera(preferred_index=args.camera_index)
    ...
    last_logged_movement_command: Optional[str] = None
    ...
    if args.disable_mqtt:
        ...
        try:
            ...
            mqtt_publisher.publish(MOVEMENT_SEARCH, force=True)
        except Exception as e:
    ...
    try:
        while True:
            ...
            for i, f in enumerate(faces):
                ...
                lock_candidate_stable = True
                current_center = _face_center_from_kps(f.kps)
                if face_lock and face_lock.last_position is not None:
                    dx_lock = current_center[0] - face_lock.last_position[0]
                    dy_lock = current_center[1] - face_lock.last_position[1]
                    if (dx_lock * dx_lock + dy_lock * dy_lock) > (args.max_lock_jump_px * args.max_lock_jump_px):
                        lock_candidate_stable = False

                if face_lock and lock_candidate_stable and mr.name == face_lock.target_name and mr.accepted:
                ...
                elif face_lock and lock_candidate_stable and face_lock.last_position is not None:
                    center_x, center_y = _face_center_from_kps(f.kps)
                    dx = center_x - face_lock.last_position[0]
                    dy = center_y - face_lock.last_position[1]
                    # When recognition flickers but the same face stays near the
                    # previous locked position, keep tracking it.
                    if (dx * dx + dy * dy) <= (args.max_lock_jump_px * args.max_lock_jump_px):
                        actions = face_lock.update_position(f.kps)
                        face_lock.history.extend(actions)
                        is_locked_face = True
                        locked_face_found = True
            ...
            if face_lock and locked_face_found and locked_face_kps is not None:
                was_reacquiring = face_missing_since is not None
                if face_missing_since is not None:
                    reacquire_count += 1
                    if reacquire_count < args.reacquire_confirm_frames:
                        movement_command = MOVEMENT_SEARCH
                        movement_error_x = 0.0
                        if movement_command != last_logged_movement_command:
                            print(f"[MQTT] Movement -> {movement_command}")
                            last_logged_movement_command = movement_command
                ...
                if was_reacquiring:
                    # Drop stale direction/smoothing state after SEARCH so the
                    # servo recenters from the face's current position instead
                    # of resuming the old direction.
                    filtered_error_x = raw_error_x
                    pending_track_command = None
                    pending_track_count = 0
                    stable_track_command = command_from_error_with_hysteresis(
                        error_x=raw_error_x,
                        deadzone_px=args.deadzone_px,
                        center_exit_hysteresis_px=args.center_exit_hysteresis_px,
                        previous_command=MOVEMENT_CENTER,
                    )
                ...
                immediate_track_threshold = float(args.deadzone_px) + float(args.center_exit_hysteresis_px) + 80.0

                if desired_track_command == stable_track_command:
                    pending_track_command = None
                    pending_track_count = 0
                elif abs(movement_error_x) >= immediate_track_threshold:
                ...
                # Hold the previous tracking command briefly across missed frames
                # so the servo does not thrash between TRACK and SEARCH.
                if lost_for < args.reacquire_hold_sec:
                    movement_command = stable_track_command
                elif args.search_when_no_faces_only and len(faces) > 0:
                    movement_command = MOVEMENT_CENTER
                # If all faces disappear entirely, begin searching immediately
                # after the reacquire hold window.
                elif len(faces) == 0 or lost_for >= args.search_delay_sec:
                    movement_command = MOVEMENT_SEARCH
                else:
                    movement_command = MOVEMENT_CENTER
                movement_error_x = 0.0
            ...
            if movement_command != last_logged_movement_command:
                if movement_command in (MOVEMENT_LEFT, MOVEMENT_RIGHT, MOVEMENT_CENTER):
                    print(f"[MQTT] Movement -> {movement_command} (err_x={movement_error_x:+.1f}px)")
                else:
                    print(f"[MQTT] Movement -> {movement_command}")
                last_logged_movement_command = movement_command
