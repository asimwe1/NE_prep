...
class ArcFaceEmbedderONNX:
    ...
    def __init__(
        ...
        if debug:
            ...
            print("[embed] input:", self.input_meta.shape)
    ...
    def _infer_embedding(self, aligned_bgr: np.ndarray) -> np.ndarray:
    ...
    def embed(self, aligned_bgr: np.ndarray) -> EmbeddingResult:
        # Average original and mirrored embeddings to reduce sensitivity to
        # glasses, small accessories, and mild pose variation.
        v = self._infer_embedding(aligned_bgr)
        v_flip = self._infer_embedding(cv2.flip(aligned_bgr, 1))
        v = 0.5 * (v + v_flip)
...
def main():
    ...
    print(f"Embedding Demo running on camera index {camera_index}. Press 'q' to quit, 'p' to print embedding.")
    t0 = time.time()
    frames = 0
    fps = 0.0
    while True:
